...
if (!require(dplyr)) {
    install.packages("purrr", repos=cranMirror);
    install.packages("crayon", repos=cranMirror);
    install.packages("dyplr", repos=cranMirror);
}
if (!require(dplyr)) {install.packages("RColorBrewer", repos=cranMirror)}
...
drugsData <- read.table(file = '../PathwayAnalysis/CellLinesExp/CCLE_NP24.2009_Drug_data_2015.02.24.tsv', sep = '\t', header = TRUE, na.strings=c('NA'))
drugData <- subset(drugsData, Compound==drug)

rownames(drugData) <- drugData$"CCLE.Cell.Line.Name"
drugDataFiltered <- subset(drugData, "CCLE.Cell.Line.Name"=colnames(df))
cellLineMetaData <- t(select(drugData, "EC50..uM.", "IC50..uM.", "Amax", "ActArea"))
cellLineMetaData <- cellLineMetaData[, sort(colnames(cellLineMetaData))]
drugMetaData <- t(cellLineMetaData)
...
pal = colorRampPalette(c("blue", "red"))(5)
#EC50colors <- cellLineMetaData["EC50..uM"]
#IC50colors <- cellLineMetaData["IC50..uM"]
#AmaxColors <- cellLineMetaData["Amax"]
#colnames(drugMetaData)
head(drugMetaData[,"ActArea"])
actAreaColors <- map2color(drugMetaData["ActArea"], pal, c(0,8))
#actAreaColors


col_annotation <- cbind(#EC50colors, IC50colors, AmaxColors,
		       	actAreaColors)
colnames(col_annotation)=c(#"EC50", "IC50", "Amax", 
			   "ActArea")
...
if (grepl("h", visual)) {
    output_htmap <- paste(output_name, "svg", sep=".")
    par(lend = 1)
    svg(output_htmap)
}
