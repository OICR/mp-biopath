...
facts("Interaction Types") do
    ...
    for (index, value) in enumerate(tests)
        a=readstring(`julia bin/mp-biopath --onenormal ./test/files/networks/$value.tsv ./test/files/observations/$value.tsv ./test/files/results/$value.tsv ./data/db_id_to_name_mapping.txt ./data/key_outputs.tsv -v`)
